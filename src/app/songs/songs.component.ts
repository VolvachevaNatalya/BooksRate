...
export class SongsComponent implements OnInit {
  ...
  randomSongsRate() {
    var rand = Math.floor(Math.random() * (10 - 1 + 1)) + 1; // generate new time (between 3sec and 500"s)
    var randIncrDecrFunc = Math.round(Math.random());
    if  (Math.round(Math.random())) {
      this.increaseRatingByOne(this.songs[rand])
    } else {
      this.decreaseRatingByOne(this.songs[rand])

    }
    console.log(randIncrDecrFunc);
